...
observed_check_unit_values <- function(dat, ledger = expected_unit_ledger) {
  ...
  values <- switch(
    ...
    aspm = if ("variant" %in% names(dat)) {
      dat$variant
    } else if ("recruitment_mode" %in% names(dat)) {
      dat$recruitment_mode
    } else if ("folder" %in% names(dat)) {
      folder <- trimws(as.character(dat$folder))
      ifelse(
        !is.na(folder) & nzchar(folder),
        ifelse(basename(folder) == "aspm", "aspm", basename(folder)),
        NA_character_
      )
    } else {
...
mfclshiny_payload_tool_env <- function(required_for) {
  ...
  # fan-in payload construction behaves like the Shiny app and package API.
