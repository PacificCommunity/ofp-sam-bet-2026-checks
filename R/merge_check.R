...
profile_reuse_input_jobs <- if (identical(check_type, "profile")) {
  split_values(env(
    ...
    env("PROFILE_CONTINUATION_SOURCE_JOBS", "")
  ))
} else {
  character()
}
...
write_merged_profile_spec <- function(root, points = NULL) {
  ...
  spec <- list(
    ...
    continuation_lower_scalar = profile_env_or_number(
      "PROFILE_CONTINUATION_LOWER_SCALAR", NA_real_
    ),
    continuation_upper_scalar = profile_env_or_number(
      "PROFILE_CONTINUATION_UPPER_SCALAR", NA_real_
    ),
